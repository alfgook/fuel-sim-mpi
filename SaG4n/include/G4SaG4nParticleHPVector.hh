...
//
#ifndef G4SaG4nParticleHPVector_h
#define G4SaG4nParticleHPVector_h 1
...
{
  friend G4SaG4nParticleHPVector & operator + (G4SaG4nParticleHPVector & left, 
                                         G4SaG4nParticleHPVector & right);
  
  public:
  
  G4SaG4nParticleHPVector();

  G4SaG4nParticleHPVector(G4int n);
  
  ~G4SaG4nParticleHPVector();
  
  G4SaG4nParticleHPVector & operator = (const G4SaG4nParticleHPVector & right);
  ...
  inline void SetPoint(G4int i, const G4SaG4nParticleHPDataPoint & it)
  {
    G4double x = it.GetX();
    G4double y = it.GetY();
    SetData(i, x, y);
  ...
  { 
//    G4cout <<"G4SaG4nParticleHPVector::SetData called"<<nPoints<<" "<<nEntries<<G4endl;
    Check(i);
  ...
  inline const G4SaG4nParticleHPDataPoint & GetPoint(G4int i) const { return theData[i]; }
  ...
  void Init(std::istream & aDataFile,G4double ux=1., G4double uy=1.)
  {
    ...
    theData = new G4SaG4nParticleHPDataPoint[total]; 
    nPoints=total;
    nEntries=0;    
    theManager.Init(aDataFile);
...
//080811 TK DB 
  ...
  inline void Merge(G4SaG4nParticleHPVector * active, G4SaG4nParticleHPVector * passive)
  {
    CleanUp();
  ...
  inline void Integrate() 
  {
    ...
    for(i=1;i<GetVectorLength();i++)
    {
      if(std::abs((theData[i].GetX()-theData[i-1].GetX())/theData[i].GetX())>0.0000001)
      {
        ...
        {
          throw G4HadronicException(__FILE__, __LINE__, "Unknown interpolation scheme in G4SaG4nParticleHPVector::Integrate");
        }
...
//  void SetBuffered(const std::vector<G4double> &aBuffer) {theBuffered = aBuffer;}
  ...
  G4SaG4nParticleHPInterpolator theLin;
  
  private:
  
  G4double totalIntegral;
  
  G4SaG4nParticleHPDataPoint * theData; // the data
