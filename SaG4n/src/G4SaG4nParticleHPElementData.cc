...
#include "G4SaG4nParticleHPElementData.hh"
  ...
  G4SaG4nParticleHPElementData::G4SaG4nParticleHPElementData()
  {
     precision = 0.02;
     theFissionData = new G4SaG4nParticleHPVector;
     theCaptureData = new G4SaG4nParticleHPVector;
     theElasticData = new G4SaG4nParticleHPVector;
     theInelasticData = new G4SaG4nParticleHPVector;
  ...
  void G4SaG4nParticleHPElementData::Init(G4Element * theElement, G4ParticleDefinition* projectile, const char* dataDirVariable )
  {
    G4int count = theElement->GetNumberOfIsotopes();
      if(count == 0) count +=
         theStableOnes.GetNumberOfIsotopes(static_cast<G4int>(theElement->GetZ()));
    theIsotopeWiseData = new G4SaG4nParticleHPIsoData[count];
...
//        G4cout <<" Init: theStableOnes in the loop: "<<A<<G4endl;
  ...
  //void G4SaG4nParticleHPElementData::UpdateData(G4int A, G4int Z, G4int index, G4double abundance)
  void G4SaG4nParticleHPElementData::UpdateData(G4int A, G4int Z, G4int M, G4int index, G4double abundance, G4ParticleDefinition* projectile, const char* dataDirVariable )
  {
    //Reads in the Data, using G4SaG4nParticleHPIsoData[], and its Init
...
//    G4cout << "ElementWiseData::UpdateData finished"<endl;
  ...
  void G4SaG4nParticleHPElementData::Harmonise(G4SaG4nParticleHPVector *& theStore, G4SaG4nParticleHPVector * theNew)
  {
    if(theNew == 0) { return; }
    G4int s_tmp = 0, n=0, m_tmp=0;
    G4SaG4nParticleHPVector * theMerge = new G4SaG4nParticleHPVector(theStore->GetVectorLength());
//    G4cout << "Harmonise 1: "<<theStore->GetEnergy(s_tmp)<<" "<<theNew->GetEnergy(0)<<G4endl;
    ...
    G4SaG4nParticleHPVector *active = theStore;
    G4SaG4nParticleHPVector * passive = theNew;
...
//    G4cout <<"Harmonise 6: "<< theStore->GetVectorLength() << " " << m << G4endl;
  ...
  G4SaG4nParticleHPVector * G4SaG4nParticleHPElementData::MakePhysicsVector(G4Element * theElement,
