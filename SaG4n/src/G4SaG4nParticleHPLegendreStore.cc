...
G4double G4SaG4nParticleHPLegendreStore::SampleDiscreteTwoBody (G4double anEnergy)
{
  G4double result;
  
  G4int i0;
  G4int low(0), high(0);
  G4SaG4nParticleHPFastLegendre theLeg;
...
G4double G4SaG4nParticleHPLegendreStore::SampleMax (G4double anEnergy)
{
  G4double result;
...
G4double G4SaG4nParticleHPLegendreStore::SampleElastic (G4double anEnergy)
{
  G4double result;
...
G4double G4SaG4nParticleHPLegendreStore::Sample (G4double energy) // still in interpolation; do not use
{
  G4int i0;
  G4int low(0), high(0);
//  G4cout << "G4SaG4nParticleHPLegendreStore::Sample "<<energy<<" "<<energy<<" "<<nEnergy<<G4endl;
  for (i0=0; i0<nEnergy; i0++)
  {
//     G4cout <<"theCoeff["<<i0<<"].GetEnergy() = "<<theCoeff[i0].GetEnergy()<<G4endl;
  ...
  low = std::max(0, high-1);
//  G4cout << "G4SaG4nParticleHPLegendreStore::Sample high, low: "<<high<<", "<<low<<G4endl;
...
//         << theBuffer.GetY(i0)/theBuffer.GetY(600)<<G4endl;;
  ...
  if(it==601) it=600;
//  G4cout << "G4SaG4nParticleHPLegendreStore::Sample it "<<rand<<" "<<it<<G4endl;
...
//  G4cout << "G4SaG4nParticleHPLegendreStore::Sample x y "<<x1<<" "<<y1<<" "<<x2<<" "<<y2<<G4endl;
  return theInt.Interpolate(theManager.GetScheme(high), rand, x1, x2, y1, y2);
}

G4double G4SaG4nParticleHPLegendreStore::Integrate(G4int k, G4double costh) // still in interpolation; not used anymore
{
  G4double result=0;
  G4SaG4nParticleHPFastLegendre theLeg;
//  G4cout <<"the COEFFS "<<k<<" ";
