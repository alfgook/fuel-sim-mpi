...
#include <iomanip>
  ...
  void G4SaG4nParticleHPList::Check(G4int i)
  {
    if(i<0) 
    {
      throw G4HadronicException(__FILE__, __LINE__, "G4SaG4nParticleHPList::Check(G4int) called with negative index");
    }
    if(i>nEntries) throw G4HadronicException(__FILE__, __LINE__, "Skipped some index numbers in G4SaG4nParticleHPList");
    if(i==nPoints)
    {
  ...
  void G4SaG4nParticleHPList::Init(std::istream & aDataFile, G4int nPar, G4double unit)
  {
    G4int i;
    G4double y;
    for (i=0; i<nPar; i++)
  ...
  void G4SaG4nParticleHPList::Init(std::istream & aDataFile, G4double unit)
  {
    G4int total, i;
    aDataFile >> total;
    G4double y;
  ...
  G4double G4SaG4nParticleHPList::GetValue(G4int i) 
  { 
...
void G4SaG4nParticleHPList::Dump()
{
