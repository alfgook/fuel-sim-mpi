...
void G4SaG4nParticleHPPhotonDist::InitAngular(std::istream & aDataFile)
{
  ...
  if (isoFlag != 1)
  {
if ( repFlag == 2 ) G4cout << "G4SaG4nParticleHPPhotonDist: repFlag == 2 && isoFlag != 1 is unexpected! If you use G4ND3.x, then please report to Geant4 Hyper News. Thanks." << G4endl;
    aDataFile >> tabulationType >> nDiscrete2 >> nIso;
//080731
      if ( theGammas != NULL && nDiscrete2 != nDiscrete ) G4cout << "080731c G4SaG4nParticleHPPhotonDist nDiscrete2 != nDiscrete, It looks like something wrong in your NDL files. Please update the latest. If you still have this messages after the update, then please report to Geant4 Hyper News." << G4endl;
...
void G4SaG4nParticleHPPhotonDist::InitPartials(std::istream & aDataFile)
{
  ...
  //G4cout << "G4SaG4nParticleHPPhotonDist::InitPartials " << G4endl;
...
*/
	    ...
	    count++;
	    if(count    > nSecondaries)  throw G4HadronicException(__FILE__, __LINE__, "G4SaG4nParticleHPPhotonDist::GetPhotons inconsistancy");
        }
