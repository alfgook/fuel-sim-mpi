...
#ifdef G4PHPDEBUG
     if( getenv("G4SaG4nParticleHPDebug") ) G4cout << "G4SaG4nParticleHPProduct::GetMultiplicity " << theMassCode << " " << theMass << " multi " << multi << " mean " << mean << G4endl;
#endif
  }
...
#ifdef G4PHPDEBUG
 if( getenv("G4PHPTEST") )
    if( getenv("G4SaG4nParticleHPDebug") && tmp != 0 )    G4cout << multi << " " << i << " @@@ G4SaG4nParticleHPProduct::Sample " << anEnergy << " Mass " << theMassCode << " " << theMass << G4endl;
#endif
    tmp = theDist->Sample(anEnergy, theMassCode, theMass);
    if(tmp != 0) { result->push_back(tmp); }
#ifndef G4PHPDEBUG //GDEB
    if( getenv("G4SaG4nParticleHPDebug") && tmp != 0 )   G4cout << multi << " " << i << " @@@ G4SaG4nParticleHPProduct::Sample " << tmp->GetDefinition()->GetParticleName() << " E= " << tmp->GetKineticEnergy() << G4endl; 
#endif
  }
  if(multi == 0) 
    {
