...
MySFDecay::MySFDecay(const G4ParticleDefinition* theParentNucleus,
 ...
 : G4NuclearDecay("SF decay", SpFission, excitationE, flb), transitionQ(Qvalue)
{
  ...
  //G4cout << "MySFDecay::MySFDecay (Z,A) = (" << parentZ << "," << parentA << ")" << G4endl;
...
G4DecayProducts* MySFDecay::DecayIt(G4double)
{
  // Fill G4MT_parent with theParentNucleus (stored by SetParent in ctor)  
  CheckAndFillParent();

  //G4cout << "MySFDecay::DecayIt()" << G4endl;
  ...
  fissionEvent *aFission = theFissionGenerator->newFissionEvent(code, 10.0, -1, 0., 0);

  G4int nNeut = aFission->getNeutronNu();
  G4int nGam = aFission->getPhotonNu();

  //G4cout << "nNeut = " << nNeut << G4endl;
  //G4cout << "nGam = " << nGam << G4endl;
...
void MySFDecay::DumpNuclearInfo()
{
  G4cout << " MySFDecay for parent nucleus " << GetParentName() << G4endl;
